...
# Multi-stage build for the DevBuilder indexer job.
#
...
#
# One-shot container: reads the raw corpus and the exported ONNX encoder,
# chunks + embeds + upserts vectors into Qdrant, then exits. The api
# container queries the same Qdrant collection at serve time.
#
...
LABEL org.opencontainers.image.description="DevBuilder indexer — chunks corpus, embeds with ONNX, upserts to Qdrant"
LABEL org.opencontainers.image.licenses="MIT"
LABEL org.opencontainers.image.title="${REPO_NAME}-indexer"
...
# Bind-mount targets at runtime: model artifacts (read-only, from export),
# raw corpus (read-only), and processed data (writable if the chunker
# materializes intermediate files). Create them so the container starts
# even when a mount is missing or read-only.
RUN mkdir -p artifacts/models data/raw data/processed \
 && chown -R app:app /app

USER app

# One-shot job: run the indexing pipeline and exit. Reads configs/settings.yaml
# for chunking params, collection name, and Qdrant connection (overridable
# via QDRANT_HOST / QDRANT_PORT env vars set by compose). The job is only
# ever run explicitly, so it rebuilds the collection rather than skipping
# when one exists.
CMD ["sh", "-c", "devbuilder --config configs/settings.yaml pipeline && devbuilder --config configs/settings.yaml index --force-recreate"]
